...
import { Mail, Phone, Slash, CheckCircle, Eye, X, CreditCard, MapPin, Calendar, Info, AlertTriangle, Award, Star, Heart, Handshake } from 'lucide-react';

const BadgeIcon = ({ name, size = 20, ...props }) => {
  switch (name) {
    case 'Handshake': return <Handshake size={size} {...props} />;
    case 'Star': return <Star size={size} {...props} />;
    case 'Heart': return <Heart size={size} {...props} />;
    default: return <Award size={size} {...props} />;
  }
};
...
const ManageUsers = () => {
  ...
  const [revealedCnics, setRevealedCnics] = useState({});
  ...
  const handleRevealCnic = async (userId) => {
    setRevealing(true);
    try {
      const res = await axios.get(`${base}/api/admin/users/${userId}/reveal-cnic`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      setRevealedCnics(prev => ({ ...prev, [userId]: res.data.cnicNumber }));
    } catch (err) {
      console.error(err);
      alert('Failed to reveal CNIC number');
    } finally {
      setRevealing(false);
    }
  };
  ...
  const fetchBadges = async () => {
    try {
      const res = await axios.get(`${base}/api/badges`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleAwardBadge = async (userId, badgeId) => {
    ...
    try {
      const res = await axios.post(`${base}/api/admin/users/${userId}/badges`, { badgeId }, {
        headers: { Authorization: `Bearer ${token}` }
      });
      const updatedBadges = res.data.badges;
      const updatedList = users.map(u => u._id === userId ? { ...u, badges: updatedBadges } : u);
      setUsers(updatedList);
      setFiltered(updatedList);
      if (selectedUser && selectedUser._id === userId) {
        setSelectedUser(prev => ({ ...prev, badges: updatedBadges }));
      }
    } catch (err) {
      console.error(err);
  ...
  const handleRevokeBadge = async (userId, badgeId) => {
    setAwardingBadgeId(badgeId);
    try {
      const res = await axios.delete(`${base}/api/admin/users/${userId}/badges/${badgeId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      const updatedBadges = res.data.badges;
      const updatedList = users.map(u => u._id === userId ? { ...u, badges: updatedBadges } : u);
      ...
      console.error(err);
      alert(err.response?.data?.msg || 'Failed to revoke badge');
    } finally {
      setAwardingBadgeId(null);
    }
  };
          ...
          <div className="admin-modal-overlay" onClick={() => setSelectedUser(null)}>
              <div className="admin-modal-content wide" onClick={(e) => e.stopPropagation()}>
                  ...
                  <div className="admin-modal-body-scroll">
                      ...
                      <div className="modal-info-grid">
                          ...
                          <div className="info-item">
                              ...
                              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                  <p style={{ margin: 0 }}>{revealedCnics[selectedUser._id] || selectedUser.cnicNumber}</p>
                                  {!revealedCnics[selectedUser._id] && (
                                      <button 
                                        onClick={() => handleRevealCnic(selectedUser._id)} 
                                        disabled={revealing}
                                        style={{
                                          padding: '2px 8px',
                                          fontSize: '11px',
                                      ...
                                      >
                                          {revealing ? 'Revealing...' : <><Eye size={12} /> Reveal</>}
                                      </button>
                                  )}
                              </div>
                          </div>
                      ...
                      {/* Performance Badges */}
                      <div className="modal-section">
                          <h4><Award size={16}/> Performance Badges</h4>
                          <div className="admin-badges-grid">
                              {systemBadges.map(badge => {
                                  const hasBadge = selectedUser.badges && selectedUser.badges.some(b => (b._id || b) === badge._id);
                                  return (
                                      <div key={badge._id} className={`admin-badge-item ${hasBadge ? 'earned' : 'locked'}`}>
                                          <div className="badge-icon-capsule">
                                              <BadgeIcon name={badge.icon} className="badge-lucide-icon" />
                                          </div>
                                          ...
                                          <div className="badge-actions">
                                              {hasBadge ? (
                                                  <button 
                                                      ...
                                                      onClick={() => handleRevokeBadge(selectedUser._id, badge._id)}
                                                      disabled={awardingBadgeId !== null}
                                                  >
                                                      {awardingBadgeId === badge._id ? 'Revoking...' : 'Revoke'}
                                                  </button>
                                              ) : (
                                                  <button 
                                                      className="btn-award-badge"
                                                      onClick={() => handleAwardBadge(selectedUser._id, badge._id)}
                                                      disabled={awardingBadgeId !== null}
                                                  >
                                                      {awardingBadgeId === badge._id ? 'Awarding...' : 'Award'}
                                                  </button>
                                              )}
                                          </div>
                                      </div>
