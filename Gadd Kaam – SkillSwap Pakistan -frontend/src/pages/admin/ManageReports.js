          ...
          <div className="admin-modal-content wide" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="case-layout">
                ...
                <div className="chat-evidence-container">
                    <div className="chat-evidence-header flex justify-between items-center px-6 py-4 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-slate-900">
                        <h4 className="flex items-center gap-2 m-0 text-slate-800 dark:text-white font-bold"><MessageSquare size={18}/> Conversation Log</h4>
                        <div className="flex gap-2">
                            <button 
                              className={`px-3 py-1.5 rounded-lg text-xs font-semibold border transition-all ${chatFilter === 'all' ? 'bg-primary-orange text-white border-primary-orange shadow-md scale-105' : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 border-gray-200 dark:border-gray-700 hover:bg-gray-50'}`}
                              onClick={() => setChatFilter('all')}
                            >
                              Show All
                            </button>
                            <button 
                              className={`px-3 py-1.5 rounded-lg text-xs font-semibold border transition-all ${chatFilter === 'accused' ? 'bg-red-500 text-white border-red-500 shadow-md scale-105' : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 border-gray-200 dark:border-gray-700 hover:bg-gray-50'}`}
                              onClick={() => setChatFilter('accused')}
                            >
                              Accused Only
                            </button>
                            <button 
                              className={`px-3 py-1.5 rounded-lg text-xs font-semibold border transition-all ${chatFilter === 'reporter' ? 'bg-blue-500 text-white border-blue-500 shadow-md scale-105' : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 border-gray-200 dark:border-gray-700 hover:bg-gray-50'}`}
                              onClick={() => setChatFilter('reporter')}
                            >
                              Reporter Only
                            </button>
                        </div>
                        ...
                        <div className="chat-history-log">
                            {(() => {
                                const filteredConversation = conversation.filter(msg => {
                                    ...
                                    return chatFilter === 'accused' ? isAccused : !isAccused;
                                });
                                ...
                                return filteredConversation.length === 0 ? (
                                    <div className="no-chat-state">
                                        ...
                                        <p>No chat messages found under this filter.</p>
                                    </div>
                                ) : (
