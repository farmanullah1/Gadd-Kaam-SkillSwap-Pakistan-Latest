...
const SkillCard = ({ skillOffer, onViewDetails, style }) => {
  ...
  return (
    <div className="skill-card reveal reveal-up premium-hover" style={style}>
      ...
      <div className="skill-card-content">
        ...
        <p className="skill-card-author"><User size={14} style={{marginRight:'4px'}}/> {authorName}</p>
        ...
        <div className="modal-scroll-content">
            ...
            <div className="modal-right-column">
                ...
                <div className="modal-author-row">
                    <span className="modal-author">{t('by_label')} <strong>{authorName}</strong></span>
                    {authorBadges.length > 0 && (
                        <div className="modal-badges-list">
                            {authorBadges.map((badge, idx) => (
                                <div key={idx} className="mini-badge" title={badge.name}><BadgeIcon name={badge.icon} size={14} /></div>
                            ))}
                        </div>
                    )}
                </div>
                ...
                <div className="details-grid">
                    <div className="detail-item"><label><Tag size={14}/> {t('swap_skill_label')}</label><p>{skillOffer.skillsToSwap?.join(', ') || t('skill_not_specified')}</p></div>
                    <div className="detail-item"><label><MapPin size={14}/> {t('location_label')}</label><p>{skillOffer.remotely ? "Remote" : skillOffer.location || t('not_specified')}</p></div>
                    <div className="detail-item"><label><Globe size={14}/> {t('remotely_label')}</label><p>{skillOffer.remotely ? t('yes') : t('no')}</p></div>
                    <div className="detail-item"><label><EyeOff size={14}/> {t('anonymous_label')}</label><p>{skillOffer.anonymous ? t('yes') : t('no')}</p></div>
                    {!skillOffer.anonymous && skillOffer.user?.phoneNumber && <div className="detail-item"><label><Phone size={14}/> {t('phone_label')}</label><p>{skillOffer.user.phoneNumber}</p></div>}
                </div>
...
function WomenOnlyZonePage({ onChatbotToggle }) {
  ...
  useEffect(() => {
    ...
    if (storedUser) {
      ...
      const alreadyChecked = localStorage.getItem('women_safety_approved');
      if (!alreadyChecked) {
    ...
    <div className="dashboard-page-container">
      ...
      <div className="dashboard-main-content">
        ...
        <section className="dashboard-content-area">
          <div className="women-only-zone-page p-6 bg-pink-50/5 dark:bg-pink-950/5 min-h-[70vh] rounded-2xl border border-pink-100 dark:border-pink-900/40 shadow-sm relative overflow-hidden backdrop-blur-md">
            <div className="women-only-zone-header flex justify-between items-center mb-6 flex-wrap gap-4 border-b border-pink-100 dark:border-pink-900/30 pb-4">
              <div>
                <h1 className="text-2xl md:text-3xl font-extrabold text-slate-800 dark:text-white bg-gradient-to-r from-pink-500 to-purple-600 dark:from-pink-400 dark:to-purple-400 bg-clip-text text-transparent">{t('women_only_zone_page_title')}</h1>
                <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">{t('women_only_zone_page_subtitle')}</p>
              </div>
              <button 
                onClick={() => {
                  setChecks({
                ...
                className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-pink-200 dark:border-pink-900 bg-pink-50/50 dark:bg-pink-950/20 text-pink-600 dark:text-pink-400 hover:bg-pink-100/50 text-sm font-bold transition-all shadow-sm hover:scale-105"
              >
                <Shield size={16} />
                Safety Guidelines
              </button>
            ...
            <div className="women-only-banner bg-gradient-to-r from-pink-50 to-purple-50 dark:from-pink-950/40 dark:to-purple-950/40 text-pink-800 dark:text-pink-200 border border-pink-200 dark:border-pink-900 p-4 mb-6 rounded-xl text-center font-bold shadow-sm">
                {t('women_only_zone_banner_text') || "Secure & Verified Space for Women"}
            </div>

            <div className="marketplace-filters-search">
              <div className="search-bar">
                <Search size={20} className="search-icon" />
                <input type="text" placeholder={t('search_skills_placeholder')} value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
              </div>
              <div className="category-filters">
                <Tag size={20} className="filter-icon" />
                {availableCategories.length > 0 ? (
                  availableCategories.map(category => (
                    <button key={category} className={`filter-tag ${selectedCategories.includes(category) ? 'active' : ''}`} onClick={() => handleCategoryToggle(category)}>
                      {category} {selectedCategories.includes(category) && <X size={14} className="clear-filter-icon" />}
                    </button>
                  ))
            ...
            {loading ? <LoadingSpinner /> : error ? <p className="error-message">{error}</p> : filteredSkills.length === 0 ? (
              <p className="no-skills-message">{allSkills.length === 0 ? t('no_skills_available_women_zone') : t('no_skills_match_filters')}</p>
            ) : (
              <div className="skill-card-container women-only-skill-card-container">
                {filteredSkills.map((offer, index) => (
                  <SkillCard key={offer._id} skillOffer={offer} onViewDetails={setSelectedSkillOffer} style={{ animationDelay: `${index * 0.1}s` }} />
                ))}
              </div>
            )}
          </div>
      ...
      {/* --- SAFETY SHIELD PROTOCOLS MODAL --- */}
      {showSafetyModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex justify-center items-center z-[3000] px-4">
          <div className="bg-white dark:bg-slate-900 border border-pink-200 dark:border-pink-900/50 rounded-2xl max-w-lg w-full p-6 shadow-2xl animate-alert-pop relative">
            <div className="flex flex-col items-center text-center mb-6">
              <div className="w-16 h-16 rounded-full bg-pink-100 dark:bg-pink-950/50 flex items-center justify-center text-pink-500 mb-4 shadow-sm animate-pulse-glow">
                <Shield size={36} />
              </div>
              <h3 className="text-xl font-extrabold text-slate-800 dark:text-white">🛡️ Women-Only Safety Protocols</h3>
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 max-w-sm">
                Please acknowledge these cashless barter safety guidelines to enter Pakistan's secure female trading zone.
              </p>
            </div>

            <div className="space-y-4 my-6">
              {[
                { key: 'publicPlace', title: 'Meet in Safe Public Spaces', desc: 'Perform any in-person swap in a crowded public place like a market, café, or library.' },
                { key: 'cnicVerified', title: 'Verify CNIC Trust Badge', desc: 'Always check if the swapper has a green CNIC verification shield on their card.' },
                { key: 'secureChat', title: 'Communicate In-App Only', desc: 'Keep all swap details and coordination inside our encrypted safe chat room.' },
                { key: 'friendAlert', title: 'Inform a Trusted Contact', desc: 'Let a friend or family member know the date, location, and person of your swap.' },
                { key: 'escrowShield', title: 'Use Secure Hour Escrow', desc: 'Ensure barter hours are locked via our secure escrow shield before exchange.' }
              ].map((item) => (
                <label 
                  key={item.key} 
                  className={`flex items-start gap-3 p-3 rounded-xl border cursor-pointer transition-all ${checks[item.key] ? 'bg-pink-50/40 dark:bg-pink-950/20 border-pink-300 dark:border-pink-800' : 'bg-transparent border-gray-200 dark:border-gray-800 hover:bg-slate-50/50'}`}
                >
                  <input 
                    type="checkbox" 
                    className="mt-1 w-4 h-4 rounded text-pink-600 focus:ring-pink-500 border-gray-300 dark:border-gray-700" 
            ...
            <div className="flex gap-3 mt-6">
              <button 
                className="w-1/3 py-3 rounded-xl text-sm font-semibold border border-gray-200 dark:border-gray-800 bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-gray-50 transition-all"
                onClick={() => { setShowSafetyModal(false); navigate('/marketplace'); }}
              >
                Go Back
              </button>
              <button 
                disabled={!Object.values(checks).every(Boolean)}
                className={`w-2/3 py-3 rounded-xl text-sm font-bold text-white transition-all shadow-md ${Object.values(checks).every(Boolean) ? 'bg-gradient-to-r from-pink-500 to-purple-600 hover:from-pink-600 hover:to-purple-700 animate-pulse-glow' : 'bg-slate-200 dark:bg-slate-800 text-slate-400 dark:text-slate-500 cursor-not-allowed border border-transparent'}`}
                onClick={() => {
                  localStorage.setItem('women_safety_approved', 'true');
